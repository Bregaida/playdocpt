...
* Since this form involves multiple method dispatching, it's slower than the raw for loo</code></pre>But the difference is in micro-second scale and probably does not matter. 
* you cannot use "{}" with the open for loo</code></pre>This is *invalid*:
<pre>
`for String name: names { 
    xxx
`}
...
First of all, please use the "Objetos implícitos available in a Play! template":http://www.playframework.org/documentation/1.1.1/templates#implicits as the background reference. 
...
# @renderArgs@: not directly available in Play! standard, not advisable to use since all parameters are supposed to be passed in via the @`args@ directive. You would also loose data typing.
