...
h2. <a>Integração contínua</a>

Now more interesing usage is to pluf cobertura with Integração contínua tools, such as "Hudson":http://hudson.dev.java.net/, can often use Cobertura results to graph the test coverage. For this purpose, the Cobertura module also generates an XML format report, in *test-result/code-coverage/coverage.xml*.
