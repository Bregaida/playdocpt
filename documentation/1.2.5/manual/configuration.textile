...
* "Conceitos principais - the conf directory":main#conf
...
See also: "Configuração de log":logs.
...
The text encoding that Play uses when communicating with the web browser and for the "Cliente Web Service":libs#WebServiceclient. You do not normally need to set this, since Play defaults to using @UTF-8@. For example:
...
See also: "Configuração SMTP":emails#smtp.
