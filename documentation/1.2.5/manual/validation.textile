...
Validações ensure that the data has certain values or meets specific requirements. You can use validation to verify that your models are correct before saving them to the database, or use them directly on HTTP parameters to validate a simple form.
...
You can also provide localisations in other languages, as described in "Internationalização":i18n. 
...
h2. <a name="custom">Validação customizada using @CheckWith</a>
...
p(note). The message syntax - @%s@, @%s2$s@ and @&{…}@ - is explained in the section on how to "Buscando mensagens traduzidas":i18n#retrieve.
