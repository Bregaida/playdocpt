...
p(note). There is a separate %(next)"Extensões de Java":javaextensions% manual page.
...
Renders a hidden input field containing a generated token that you can use in any form, to prevent "Site avaliado como falso":security#csrf.
